...
class TestMadlib(unittest.TestCase):
    ...
    def test_replace_parts_of_speech(self):
        ...
        ####### ADDED F20: check student is not replacing all POS
        line = 'The PLURAL NOUN VERB past the ADJECTIVE NOUN.'
        with patch('builtins.input', 
              side_effect=['roadrunner']):
            line = replace_parts_of_speech(line, 'PLURAL NOUN')
        self.assertEqual('The roadrunner VERB past the ADJECTIVE NOUN.', line)
        # test the prompt
        line = 'The NOUN.'
    ...
    def test_print_report(self):
        ...
        ####### ADDED FA20: catch opening report.txt files
        report = inspect.getsource(print_report)
        self.assertTrue('report.txt' not in report)
        self.assertTrue('report_correct.txt' not in report)
        sys.stdin = save_in
        sys.stdout = save_out
        infile.close()
        outfile.close()
    ...
    def test_main(self):
        ...
        with io.StringIO() as buf, redirect_stdout(buf):
            try:
                ...
                pass
            prompt = buf.getvalue()
            ####### UPDATED SUM20: fixes bug with getting prompt
            self.assertTrue("Enter file name: " in prompt) 
        sys.stdin = sys.__stdin__
